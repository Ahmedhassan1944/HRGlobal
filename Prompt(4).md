...
## 5. Mandatory hover and KSA regression fix

The latest rendered result shows two blocking defects that must be fixed before visual acceptance:

1. When the pointer hovers over a Middle East country, the country labels disappear.
2. Saudi Arabia/KSA is visible as a label but its polygon is not receiving the required active fill `#b38126`.

Treat both defects as implementation bugs, not as acceptable visual behavior.

### Labels must persist during hover

Render the country polygons, labels, and tooltip in separate SVG layers. The label layer must be rendered after the country-path layer so labels remain visible above the geography. Hovering or focusing a country may change only that country’s outline, stroke width, or a subtle highlight state. It must never hide, remove, replace, fade, or rebuild the labels.

Explicitly verify that no hover or focus handler sets the labels layer or any label element to `display: none`, `visibility: hidden`, `opacity: 0`, an empty text value, or an obsolete z-index. Keep labels in a dedicated group such as `#map-labels-layer`, use `pointer-events: none` for the label badges where appropriate, and keep the tooltip in a separate layer/container. A tooltip must not be implemented by temporarily removing the permanent country label.

Test the exact interaction sequence in the actual rendered map: hover Egypt, Saudi Arabia, UAE, Oman, move between each country, move outside the map, and focus the countries with the keyboard. All six permanent labels must remain visible and readable throughout the sequence. The label for Saudi Arabia must continue to read exactly `Saudi Arabia`.

### Saudi Arabia polygon fill must persist in every state

Trace the complete code path from `mapPins` through canonical country resolution to the actual SVG path. The Saudi Arabia feature must have a stable attribute such as `data-iso3="SAU"` and must be found through the same canonical code used by the active-data map. When `mapPins` contains `KSA` or `Saudi Arabia` with a positive headcount, the renderer must set the Saudi path fill directly to `#b38126`, not merely add a class whose CSS can be overridden.

Use a deterministic final style pass after paths and labels are created and after every `updatePins()` call. The active-state logic must be equivalent to:

```javascript
var isSaudiActive = Number(activeData.SAU || 0) > 0;
var saudiPath = pathElementsByIso3.SAU;
if (!saudiPath) throw new Error('Required SAU path is missing');
saudiPath.setAttribute('fill', isSaudiActive ? '#b38126' : FILL_CONTEXT);
saudiPath.style.fill = isSaudiActive ? '#b38126' : FILL_CONTEXT;
```

Adapt the code to the existing implementation, but preserve this behavior. Ensure hover enter/leave, focus/blur, tooltip rendering, label rendering, zoom, pan, and rerendering cannot reset the active Saudi polygon to the context color. If a CSS rule overrides the fill, fix the selector/cascade rather than hiding the problem; use `!important` only as a last-resort, documented safeguard.

Add a development assertion after `updatePins()` and after Saudi hover/focus transitions. When the Saudi headcount is positive, assert that the `SAU` path exists and that its effective fill is `#b38126` (or the exact equivalent returned by `getComputedStyle`). The assertion must fail loudly in development if KSA is still dark. Do not report completion based only on the presence of the Saudi label.
...
11. Hovering Egypt, Saudi Arabia, UAE, and Oman keeps all permanent labels visible.
12. Local zoom/pan works if implemented.
13. Page switching and `invalidateSize` compatibility do not throw errors.
14. Resizing the browser preserves a readable map.
15. DevTools Network shows no requests to OSM, CARTO, Azure Maps, Mapbox, Google Maps, ArcGIS, geocoding services, external GeoJSON, or any other map endpoint.
16. The browser console contains no map-related errors or failed tile requests.
...
- Hovering or focusing any Middle East country never hides or removes any permanent country label; all six labels remain visible and readable throughout hover transitions.
