...
Increase the country-name labels so they are clearly readable at the actual desktop map size. The current labels are too small. Increase the visible name text by approximately **4 CSS pixels**, or use the nearest larger size that is comfortably readable without zooming. Because the SVG uses a `700 × 700` viewBox, do not blindly add four SVG units; verify the rendered screen size. As a practical starting point, increase the current country-name font from `7` SVG units to approximately `12–14` SVG units, or the equivalent CSS-rendered size of approximately `11–13px`, then adjust after viewing the actual map.

Apply the readable size consistently to all six country names: `Egypt`, `Saudi Arabia`, `UAE`, `Oman`, `Tanzania`, and `Angola`. Keep the headcount text visibly subordinate but readable. Increase the label pill height, corner radius, and horizontal padding as needed to contain the larger text. The text must be vertically and horizontally centered, must not be clipped, and must not overlap neighboring labels. The longer `Saudi Arabia` label must receive enough width for the full phrase. Validate the result at the normal map-container size, not only after zooming in.
...
- All six country-name labels use a clearly readable size, approximately 4 CSS pixels larger than the current implementation or the nearest suitable size, without clipping or overlap.
...
6. Confirmation that the label reads `Saudi Arabia` and is readable at the normal map size.
7. Confirmation that all six country-name labels were enlarged and remain uncut and non-overlapping.
8. Hover/focus test results proving labels remain visible.
9. Click/filter compatibility results.
10. Network verification showing no external map requests.
11. Any test limitations. Do not fabricate browser, offline, network, or licensing results.
