...
3. Egypt, KSA/Saudi Arabia, UAE, Oman, Tanzania, and Angola resolve correctly.
4. Active countries use fill exactly `#b38126`.
5. Non-target countries remain dark context.
6. Labels render locally.
7. Headcount comes from the real application payload.
8. Hover tooltip works without a network request.
9. Click/selection works through existing state/callbacks.
10. Local zoom/pan works if implemented.
11. Page switching and `invalidateSize` compatibility do not throw errors.
12. Resizing the browser preserves a readable map.
13. DevTools Network shows no requests to OSM, CARTO, Azure Maps, Mapbox, Google Maps, ArcGIS, geocoding services, external GeoJSON, or any other map endpoint.
14. The browser console contains no map-related errors or failed tile requests.
...
- The map looks substantially closer to the Power BI reference than the current Leaflet screenshot.
